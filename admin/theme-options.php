...
require get_template_directory() . '/admin/inc/style.php';
...
/**
 * Beauty オプションTools デフォルト画像設定フィルター
 */
function beauty_theme_options_tools_get_default_images_settings( $default_images_settings ) {
	// デフォルト画像設定
	return array(
/*
		array(
...
*/
		array(
			'media_filename' => 'beauty-op_top-slider.gif',
			'source_filepath' => get_template_directory() . '/assets/images/op_default/op_top-slider.gif',
			'theme_option_keys' => array( 'header_slider_img1', 'header_slider_img2', 'header_slider_img3' )
		),
		array(
			'media_filename' => 'beauty-op_sp_top-slider.gif',
			'source_filepath' => get_template_directory() . '/assets/images/op_default/op_sp_top-slider.gif',
			'theme_option_keys' => array( 'header_slider_img_sp1', 'header_slider_img_sp2', 'header_slider_img_sp3' )
		)
	);
}

/**
 * Beauty オプションTools サンプル記事
 */
function beauty_theme_options_tools_set_sample_posts( $dp_options ) {
  ...
  //$media_id['news'] = theme_options_tools_get_media_id( array(
	...
	$media_id['staff'] = theme_options_tools_get_media_id( array(
		'media_filename' => 'beauty-image_760x1030.gif',
		'source_filepath' => get_template_directory() . '/assets/images/op_default/image_760x1030.gif'
	), true );

	$media_id['style'] = theme_options_tools_get_media_id( array(
		'media_filename' => 'beauty-image_370x500.gif',
		'source_filepath' => get_template_directory() . '/assets/images/op_default/image_370x500.gif'
	), true );

	$media_id['slider_image'] = theme_options_tools_get_media_id( array(
		'media_filename' => 'beauty-cf_760x1030.gif',
		'source_filepath' => get_template_directory() . '/assets/images/op_default/cf_760x1030.gif'
	), true );
  ...
  for ( $i = 4; $i >= 1; $i-- ) {
    ...
    $sample_posts[] = array(
      ...
      'thumbnail' => intval( $media_id['post'] )
		...
		$sample_posts[] = array(
			'post_title' => __( 'Sample staff', 'tcd-w' ) . $i,
			'post_name' => 'sample-staff' . $i,
			'post_status' => 'publish',
			'post_type' => 'staff',
      'thumbnail' => intval( $media_id['staff'] ),
      'menu_order' => $i,
      'metas' => array( 
				'staff_comment' => __( 'sample text sample text. sample text sample text.', 'tcd-w' ) 
      )
		);
		$sample_posts[] = array(
			'post_title' => __( 'Sample style', 'tcd-w' ) . $i,
			'post_name' => 'sample-style' . $i,
			'post_status' => 'publish',
			'post_type' => 'style',
      'thumbnail' => intval( $media_id['style'] ),
      'menu_order' => $i,
      'metas' => array( 
        'slider1_image' => intval( $media_id['slider_image'] ),
        'slider2_image' => intval( $media_id['slider_image'] ),
        'slider3_image' => intval( $media_id['slider_image'] ),
        'slider4_image' => intval( $media_id['slider_image'] )
      )
		);
	}
...
/**
 * Beauty オプションTools サンプルメニュー
 */
function beauty_theme_options_tools_set_sample_menus( $dp_options ) {
  ...
  for ( $i = 1; $i <= 6; $i++ ) {
	...
	foreach( $menu_items as $menu_item ) {
		...
		$menu_item_db_id = wp_update_nav_menu_item( $menu_id, 0, array(
			...
			'menu-item-description' => $menu_item['desc'],
...
/**
 * Beauty オプションTools サンプルウィジェット
 */
function beauty_theme_options_tools_set_sample_widgets( $dp_options ) {
  ...
  // バナーウィジェットのダミー画像の ID を取得
  $banner_image= theme_options_tools_get_media_id( array(
    'media_filename' => 'beauty-image_300x140.gif',
		'source_filepath' => get_template_directory() . '/assets/images/op_default/image_300x140.gif'
	), true );

  // バナーウィジェットのパラメーター配列を組み立てる
  $banner_widget_params['title'] = '';
  for ( $i = 1; $i <= 10; $i++ ) {
    $banner_widget_params['banner_title' . $i] = '';  
    $banner_widget_params['banner_sub' . $i] = '';  
    $banner_widget_params['banner_color' . $i] = '#000000';  
    $banner_widget_params['banner_url' . $i] = '#';  
    $banner_widget_params['banner_target_blank' . $i] = '';  
    $banner_widget_params['banner_image' . $i] = $i <= 5 ? $banner_image : ''; // 5枚の画像を追加
  }
	...
	$sample_widgets = array(
		...
		array(
			...
			'params' => array(
				...
				'banner_image1' => 	theme_options_tools_get_media_id( array(
					'media_filename' => 'beauty-image_300x250.gif',
		...
		// バナーウィジェット
		array(
			'class_name' => 'tcdw_banner_list_widget',
			'params' => $banner_widget_params
		)
	);
 ...
 * Beauty テーマ初期化フィルター
 */
add_filter( 'tcd_theme_options_tools-get_default_images_settings', 'beauty_theme_options_tools_get_default_images_settings', 10, 1 );
//add_filter( 'tcd_theme_options_tools-initialize', 'beauty_theme_options_tools_set_sample_categories', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'beauty_theme_options_tools_set_sample_posts', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'beauty_theme_options_tools_set_sample_menus', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'beauty_theme_options_tools_set_sample_widgets', 10, 1 );
