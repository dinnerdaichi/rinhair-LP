...
if ( is_mobile() ) {
	...
	if ( $options[$slug . '_ad_code3'] || $options[$slug . '_ad_image3'] || $options[$slug . '_ad_code4'] || $options[$slug . '_ad_image4'] ) {
		echo '<div class="p-entry__ad p-entry__ad--lower">' . "\n";
		if ( $options[$slug . '_ad_code3'] ) {
			echo '<div class="p-entry__ad-item">' . $options[$slug . '_ad_code3'] . '</div>';
		} elseif ( $options[$slug . '_ad_image3'] ) {
			$image3 = wp_get_attachment_image_src( $options[$slug. '_ad_image3'], 'full' ); 
			echo '<div class="p-entry__ad-item"><a href="' . esc_url( $options[$slug. '_ad_url3'] ) . '"><img src="' . esc_attr( $image3[0] ) . '" alt=""></a></div>';
		}
		if ( $options[$slug . '_ad_code4'] ) {
			echo '<div class="p-entry__ad-item">' . $options[$slug . '_ad_code4'] . '</div>';
		} elseif ( $options[$slug . '_ad_image4'] ) {
			$image4 = wp_get_attachment_image_src( $options[$slug . '_ad_image4'], 'full' ); 
			echo '<div class="p-entry__ad-item"><a href="' . esc_url( $options[$slug . '_ad_url4'] ) . '"><img src="' . esc_attr( $image4[0] ) . '" alt=""></a></div>';
		}
		echo '</div>' . "\n";
	}
}
